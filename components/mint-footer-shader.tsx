...
interface MintFooterShaderProps {
  proportion?: number
  softness?: number
  distortion?: number
  swirl?: number
...
export default function MintFooterShader({
  proportion = 0.45,
  softness = 1.0,
  distortion = 0.8,
  swirl = 0.6,
  ...
  colors = [
    ...
    "#A2D5C6", // pastel-green
    "#F6F6F6", // light-gray
    "#E8F5F1", // soft mint variation
  ],
  className = "",
}: MintFooterShaderProps) {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.error("WebGL2 not supported for mint footer shader")
      return
    }
    glRef.current = gl
    ...
    void main(){
      ...
      col = mix(col, vec3(1.0, 0.98, 1.0), highlight);
  ...
  }, [proportion, softness, distortion, swirl, swirlIterations, shapeScale, scale, rotation, speed, colors])

  return <canvas ref={canvasRef} className={`block w-full h-full ${className}`} style={{ background: "#E8F5F1" }} />
}

export { MintFooterShader }
